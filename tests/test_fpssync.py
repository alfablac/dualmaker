...
class FPSDecisionTests(unittest.TestCase):
    ...
    def test_telecine_editorial_duration_does_not_prove_pal_speed(self) -> None:
        """A 1000:1001 audio-duration candidate must not enable 0.8x audio retiming."""
        config = DualMakerConfig(allow_experimental_fps_sync=True)
        decision = evaluate_fps_pair(
            asset("master.mkv", 1000, FrameRate(24000, 1001)),
            asset("dual.mkv", 966.32, FrameRate(30000, 1001)),
            config,
        )
        weak_samples = [FPSMatchSample(0.5, 500, 500, 0.3)]
        ...
        def weak_hypothesis(
            *_args: object, speed_factor: float, **_kwargs: object
            ...
            return _Hypothesis(
                speed_factor=speed_factor,
                # Make the duration-nominated PAL candidate look stronger,
                # as in the Walker report, without reliable content matches.
                confidence=0.6 if abs(speed_factor - 0.96) < 0.001 else 0.3,
                samples=weak_samples,
            )
        ...
        with (
            ...
            result = analyze_fps_timing(
                ...
                Path("master.mkv"),
                duration=966.32,
                source_duration=966.32,
                decision=decision,
                config=config,
                work_dir=Path("."),
                runner=object(),  # type: ignore[arg-type]
                source_original_duration=966.32,
                target_original_duration=1000,
            )
